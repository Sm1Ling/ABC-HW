...
PrintStart:
        invoke printf,str1       ;invoke - "Вызов функции из библиотеки"  - после названия функции аргументы через запятую
        add esp,4            ;Я ЕБАЛ msvcrt.dll (printf scanf ...) потратил на это 50% времени, это действие должна делать функция где-то внутри себя, но почемуто msvcrt не делает этого и приходится самому, без этого тупо не пашет
...
AllocArray:           ;Выделение памяти под первый массив
        ...
        shl eax,2           ;хз проходили ли вы shl - умножение на степень двойки (eax=eax*2^2 : eax=eax*4) так как нам надо кол-во_ячеек*4, так как размер ячейки - Int = 4 байта
